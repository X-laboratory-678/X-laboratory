...
| Vacancies and funding routes | `/docs/home/vacancies/` | Covered | The Opportunity entity, effective deadline state, archive UI, official admissions guidance, and honest bilingual empty state are implemented; no real opening is currently verified. | Publish records only from authoritative vacancy sources and keep the current zero-opening state until then. |
...
| Grants portfolio | `/docs/research/grants/` | Covered | Separate bilingual Grant Page Bundles, controlled status, relationship validation, active/completed grouping, and an honest zero-record state are implemented. | Publish only verified award records; Projects remain separate entities. |
| Completed-grants disclosure | `/docs/research/grants/` | Covered | The accessible disclosure renders when completed records exist and stays absent for the current empty dataset. | Preserve native `<details>` and server-rendered records. |
...
Phase B status: bilingual Grants and Opportunities directories, details, empty states, archive behavior, deadline-derived opportunity state, homepage featured aggregators, validation, and disabled-in-production draft fixtures are implemented. Production Grants and Opportunities both remain at zero verified records.
